...
class AxionVueOpenAPI:
    ...
    def __send_ws_message(self, msg: dict, count: int = 0):
        ...
        try:
            print(f"Sended messagge: {json.dumps(msg, separators=(',', ':'))}")
    ...
    def __send_ws_message_fast(self, msg: dict, count: int = 0):
        if count == 100:
            raise RecursionError(f"Retried to send a message {count} times")
        try:
            self.ws.send(json.dumps(msg, separators=(',', ':')))
    ...
    def move_stage(
        ...
        print(f"Manually steeing is_moving to True")
        msg = {
        ...
        moving_time = 0.3 + max_waiting_time
        print(f"Moving stage to ({new_x}, {new_y}). Assuming movement will take {moving_time} seconds.")
        time.sleep(moving_time)  # Assume the movement takes at least 5 seconds

        device.x = float(new_x)
        device.y = float(new_y)

        # Stop movement assumption
        device.is_moving = False
        print(f"Manually setting is_moving to False after {moving_time} seconds.")
        print(f"Device position is {self.__all_devices[serial_number].x, self.__all_devices[serial_number].y}")
    ...
    def move_stage_fast(
            ...
            self.__send_ws_message_fast(msg)
            moving_time = 0.3 + max_waiting_time
            time.sleep(moving_time)  # Assume the movement takes at least 5 seconds
    ...
    def get_image_fast(self, serial_number: str) -> Image.Image:
        """
        Get the current image of the camera.
