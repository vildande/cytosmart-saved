...
class Listener(Thread):
    ...
    def run(self):
        while True:
            ...
            # print("RECEIVED websocket message:", new_message) # debugging
            if new_message == None:
                time.sleep(0.5)
                continue
    ...
    def __update_is_moving(self, serial_number: str, is_moving_state: bool):
        device = self.all_devices.get(serial_number, None)
        if device:
            if isinstance(is_moving_state, bool):  # Check if it's a boolean
                device.is_moving = is_moving_state
            elif str(is_moving_state).lower() == "false":
                device.is_moving = False
            elif str(is_moving_state).lower() == "true":
